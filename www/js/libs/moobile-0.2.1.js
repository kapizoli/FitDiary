...
Moobile.Button = new Class({
	...
	willBuild: function() {
		...
		/*var css = '.'+this.getName()+'.is-highlighted:before{background-image: url(img/'+this.getName()+'-on.png) !important;} .'+this.getName()+':before { 	content: "&nbsp;";	display:block;	text-indent: -1000px;	background-image: url(img/'+this.getName()+'.png) !important;background-repeat: no-repeat;	background-size: 32px 32px;	background-position: left center;	width: 32px;	height: 32px;	margin-right: 10px; }',
	...
	setImage: function(image) {
		...
		this._image = image;
		this._image.addClass('button-image');
		this.toggleClass('button-image-empty', this._image.isEmpty());
...
Moobile.List = new Class({
	...
	setMultiSelectable: function(boolVar){
		this._multiSelectable=boolVar;
	},
	...
	setCheckedItem: function(selectedItem) {

		if (this._selectable == false)
			return this;

		if (this._checkedItems.indexOf(selectedItem)===-1){
			//this.fireEvent('select', selectedItem);
			selectedItem.setSelected(true);
			this._checkedItems.push(selectedItem);
		}else{
			//this.fireEvent('deselect', selectedItem);
			selectedItem.setSelected(false);
			this._checkedItems.splice(this._checkedItems.indexOf(selectedItem),1);
		}
	...
	_onItemTap: function(e, sender) {
		if (this._multiSelectable=== false){
			if (this._selectable) this.setSelectedItem(sender);
		} else {
			if (this._selectable) this.setCheckedItem(sender);
		}
	}
